...
class Evaluate:
    ...
    def get_single_chess_point(self, chess: Chess):
        ...
        if chess.team == self.team:
            return self.single_chess_point[chess.name]  # Ai的棋子在重要的位置会增加Ai的得分
        else:
            return -1 * self.single_chess_point[chess.name]  # 人类的棋子在重要的位置会减少Ai的得分

    def get_chess_pos_point(self, chess: Chess):
        red_pos_point_table = self.red_pos_point[chess.name]
        if chess.team == 'r':  # 是红方就直接算红方的位置
            pos_point = red_pos_point_table[chess.row][chess.col]
        else:  # 是黑方就算将黑方的位置对称到红方的棋盘上计算位置
            pos_point = red_pos_point_table[9 - chess.row][chess.col]  # 这里虽然有10行，但是索引是从0开始的，所以中线在4.5
        if chess.team != self.team:
            pos_point *= -1  # 同样，人类的棋子在重要的位置上会减少Ai的得分
        return pos_point
            ...
            for cs in chess_list:
                ...
                if depth % 2 == 1 and cs.team == self.team:
                    ...
                    for new_x, new_y in next:
                        ...
                        a = max(a, temp)  # 因为max层是取最大的，所以又因为上一层要最小值，所以之后的就不用看了直接剪枝
                        if b <= a:  # 剪枝
                            return a
                            # 该层为min
